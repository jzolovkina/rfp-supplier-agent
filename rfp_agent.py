...
Run: streamlit run rfp_agent.py
"""
...
with st.sidebar:
    ...
    supplier_count_override = st.number_input("Suppliers per RFP (override)", min_value=0, max_value=50, value=0,
        help="Set to 0 to use the value from the Excel file. Start with 3–5 to test quality first.")
    st.divider()
...
def parse_excel(file_bytes):
    ...
    if "RFP_Input" not in wb.sheetnames:
        st.error('Sheet "RFP_Input" not found.'); return [], []
    ws = wb["RFP_Input"]
    rows_raw = list(ws.iter_rows(values_only=True))
    header_row_idx = -1
    for i, row in enumerate(rows_raw):
        if str(row[0] or "").strip().lower().replace(" ", "_") == "rfp_id":
...
headers = [_re.sub(r'[^a-z0-9]+', '_', str(c or "").strip().lower()).strip("_")
    ...
    for row in rows_raw[header_row_idx + 2:]:
        ...
        if not rid or re.match(r"^(unique|e\.g\.|links|\s*$)", rid, re.I): continue
        sc = get(row, "supplier_count")
        rfp_rows.append({
            "rfp_id": rid, "title": get(row, "rfp_title", "title"),
            "description": get(row, "description"), "country": get(row, "country"),
            "region": get(row, "region_city", "region", "city"),
            "phase": get(row, "rfp_phase", "phase"),
            "keywords": get(row, "keywords_tags", "keywords_/_tags", "keywords", "tags"),
            "supplier_count": int(sc) if sc.isdigit() else 20,
            "notes": get(row, "notes"),
    ...
    if "SPEC_Items" in wb.sheetnames:
        ...
        for i, row in enumerate(rows2):
            if str(row[0] or "").strip().lower() == "rfp_id": h2_idx = i; break
        if h2_idx >= 0:
            heads2 = [str(c or "").strip().lower().replace(" ","_").replace("/","_") for c in rows2[h2_idx]]
            i2 = {h: i for i, h in enumerate(heads2)}
            for row in rows2[h2_idx + 2:]:
                sid = str(row[i2.get("rfp_id", 0)] or "").strip()
                if not sid or re.match(r"links to|rfp_id", sid, re.I): continue
                spec_rows.append({
                    "rfp_id": sid,
...
def call_claude(client, prompt, max_tokens, single_object):
    ...
    if single_object:
        ...
        if not m: raise ValueError(f"No JSON object. Preview: {full_text[:200]}")
        return json.loads(m.group())
    else:
        m = re.search(r"\[[\s\S]*\]", full_text)
        if not m: raise ValueError(f"No JSON array. Preview: {full_text[:200]}")
        result = json.loads(m.group())
        if not isinstance(result, list): raise ValueError("Response is not a list.")
        return result
...
def find_companies(client, rfp, specs, count):
    ...
    spec_text = ""
    if has_spec:
        lines = [f"  {i+1}. {s['item_name']}" + (f" | Qty: {s['quantity']} {s['unit']}" if s["quantity"] else "")
...
{'NOTE: No specification — use title, description, keywords.' if not has_spec else ''}

Return ONLY a JSON array with {count} objects. No explanation, no markdown.
...
MID_BLUE, DARK_BLUE, LIGHT_BLUE, WHITE, GREY_BG = "2E5FA3","1F3864","D6E4F0","FFFFFF","F2F2F2"
...
def build_output_excel(original_bytes, rfp_rows, results):
    ...
    HEADERS = ["#","Company Name","Website","Country","City","Contact Person","Job Title",
               "Direct Phone","Direct Email","LinkedIn","General Email","General Phone",
               "Company Description","Why Relevant","Relevance Score (1-10)","Persona Match?","Contact Notes"]
    ...
    for rfp in rfp_rows:
        ...
        banner = ws["A1"]
        banner.value = (f"SUPPLIER LIST FOR SSM  |  ID {rfp['rfp_id']}: {rfp['title']}"
                        f"  |  {rfp['country']}" + (f"  |  Phase: {rfp['phase']}" if rfp["phase"] else ""))
        ...
        ctx = ws["A2"]
        ctx.value = (f"Keywords: {rfp['keywords']}  |  Companies found: {len(suppliers)}  |  "
                     f"With persona contact: {sum(1 for s in suppliers if s.get('contact_found'))}")
        ...
        if not suppliers:
            ...
            for ri, s in enumerate(suppliers):
                ...
                row_data = [ri+1, s.get("company_name",""), s.get("website",""), s.get("country",""),
                            s.get("city",""), s.get("contact_person",""), s.get("job_title",""),
                            s.get("phone",""), s.get("email",""), s.get("linkedin",""),
                            s.get("general_email",""), s.get("general_phone",""),
                            s.get("company_description",""), s.get("why_relevant",""),
...
uploaded = st.file_uploader("Drop your RFP_Supplier_Template.xlsx here", type=["xlsx","xls"],
...
if uploaded:
    ...
    if rfp_rows:
        ...
        import pandas as pd
        preview = pd.DataFrame([{"RFP ID": r["rfp_id"], "Phase": r["phase"] or "—",
            "Title": r["title"][:60]+"…" if len(r["title"])>60 else r["title"],
            "Country": r["country"], "Keywords": r["keywords"][:50], "Suppliers": r["supplier_count"]}
            for r in rfp_rows])
        st.dataframe(preview, use_container_width=True, hide_index=True)

st.divider()
can_run = bool(rfp_rows) and bool(api_key) and api_key.startswith("sk-")
if not api_key: st.info("👈 Enter your Anthropic API key in the sidebar to get started.")
elif not api_key.startswith("sk-"): st.error("API key must start with sk-")
elif not rfp_rows: st.info("Upload an RFP Excel file above to continue.")
...
if st.button("▶ Find Suppliers", type="primary", disabled=not can_run):
    ...
    def add_log(msg):
        ...
        log_area.markdown(
            ...
            "padding:12px 16px;font-family:monospace;font-size:12px;max-height:300px;"
            "overflow-y:auto;line-height:1.8;color:#8b949e'>"
            + "<br>".join(log_lines[-30:]) + "</div>", unsafe_allow_html=True)
    ...
    for i, rfp in enumerate(rfp_rows):
        ...
        add_log(f"  📋 {len(specs)} spec items" if specs else "  ℹ No specification")
        add_log(f"  ⚡ [Stage 1] Searching {count} companies…")
        companies = []
        try:
            companies = find_companies(client, rfp, specs, count)
            add_log(f"  ✓ [Stage 1] {len(companies)} companies found")
        ...
        for j, co in enumerate(companies):
            try:
                ...
                enriched.append({**co, "contact_found":False, "contact_person":"", "job_title":"",
                                  "email":"", "phone":"", "linkedin":"", "general_email":"",
                                  "general_phone":"", "contact_note":"error"})
            if j < len(companies)-1: time.sleep(1.5)
        results[rfp["rfp_id"]] = enriched
        wc = sum(1 for s in enriched if s.get("contact_found"))
        add_log(f"  ✓ [Stage 2] Persona contacts: {wc}/{len(enriched)}")
        if i < total-1: add_log("  ⏳ Pausing 5s…"); time.sleep(5)
    ...
    for i, rfp in enumerate(rfp_rows):
        ...
        with cols[i % len(cols)]:
            st.metric(label=f"{rfp['rfp_id']}", value=f"{len(sup)} companies", delta=f"{wc} with contact")
    st.divider()
    with st.spinner("Building Excel file…"):
        out_bytes = build_output_excel(original_bytes, rfp_rows, results)
    from datetime import date
    st.download_button(label="⬇ Download Excel for SSM", data=out_bytes,
